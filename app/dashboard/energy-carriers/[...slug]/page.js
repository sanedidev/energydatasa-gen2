...
export default function DynamicCarrierPage({ params }) {
    const { slug } = usePromise(params);
    const slugPath = slug.join(".");
    const nodeConfigSlug = `dyn.node.dashboard.energy-carriers.${slugPath}`;
    ...
    if (!config) {
        return (
            <div className="py-16 text-center">
                ...
                <Link href="/dashboard/energy-carriers" className="mt-4 inline-block text-sm text-green-700 hover:underline">
                    ← Back to Energy Carriers
                </Link>
            </div>
        );
    }
    ...
    if (config.type === "carrier") {
        return (
            <CarrierIndexPage
                pageSlug={`dyn.cards.dashboard.energy-carriers.${slugPath}`}
                defaultCards={[]}
                crumbs={crumbs}
                label="Datasets"
                title={config.title}
    ...
    if (config.type === "file") {
        return (
            <div className="space-y-8">
                ...
                <EditableSections
                    pageKey={`dyn.file.dashboard.energy-carriers.${slugPath}`}
