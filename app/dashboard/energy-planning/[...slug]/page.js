...
export default function DynamicEnergyPlanningPage({ params }) {
    const { slug } = usePromise(params);
    const slugPath = slug.join(".");
    const nodeConfigSlug = `dyn.node.dashboard.energy-planning.${slugPath}`;
    ...
    if (!config) {
        return (
            <div className="py-16 text-center">
                ...
                <Link href="/dashboard/energy-planning" className="mt-4 inline-block text-sm text-green-700 hover:underline">
                    ← Back to Energy Planning
                </Link>
            </div>
        );
    }
    ...
    const crumbs = config.crumbs ?? [
        ...
        { label: "Energy Planning", href: "/dashboard/energy-planning" },
        { label: config.title },
    ];
    ...
    if (config.type === "carrier") {
        return (
            <CarrierIndexPage
                pageSlug={`dyn.cards.dashboard.energy-planning.${slugPath}`}
                defaultCards={[]}
                crumbs={crumbs}
                label="Datasets"
                title={config.title}
    ...
    if (config.type === "file") {
        return (
            <div className="space-y-8">
                ...
                <EditableSections
                    pageKey={`dyn.file.dashboard.energy-planning.${slugPath}`}
